...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.sum_img_y = tf.reshape(self.y[0,:,:,0],shape=[1,tf.shape(self.y)[1],tf.shape(self.y)[2],1])
        self.sum_img_y2 = tf.reshape(self.y[0,:,:,1],shape=[1,tf.shape(self.y)[1],tf.shape(self.y)[2],1])
        self.sum_img_pred = tf.reshape(self.predicter[0,:,:,0],shape=[1,tf.shape(self.y)[1],tf.shape(self.y)[2],1])
        self.sum_img_pred2 = tf.reshape(self.predicter[0,:,:,1],shape=[1,tf.shape(self.y)[1],tf.shape(self.y)[2],1])       
        
        eps = 1e-5
        intersection = tf.reduce_sum(self.predicter[:,:,:,1] * self.y[:,:,:,1])
        union =  eps + tf.reduce_sum(self.predicter[:,:,:,1]) + tf.reduce_sum(self.y[:,:,:,1])
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False, write_graph=False):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.output_minibatch_stats(sess, test_writer, 0, test_x, util.crop_to_shape(test_y, pred_shape)) 
            avg_gradients = None
            for epoch in range(epochs):
                ...
                self.output_minibatch_stats(sess, test_writer, (epoch+1)*training_iters, test_x, util.crop_to_shape(test_y, pred_shape))
